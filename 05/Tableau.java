...
public class Tableau {
    ...
    public void addValue(Integer value) {
        ...
        // empty Tableau - addToRow to add value

        
        Cell curr = smallest;
        Integer bumpedValue = value;//addToRow(smallest , value);
        
       
        while (bumpedValue != null) {
            bumpedValue =  addToRow(curr, bumpedValue);
            //System.out.println("Bumped value "+ bumpedValue);
    ...
    protected Integer addToRow(Cell curr, int value) {
        ...
        while (curr != null) { // while right cell is not null 
            if (value < curr.value){
                int a = curr.value;// returns bumped value
                curr.value = value; // value replace value on current cell
                return a;
            } else if(curr.right == null && value > curr.value){
                //reach the end of the row, add value into a new cell
                Cell newCell = new Cell(value); // otherwise a new  is created
                curr.right = newCell;
                newCell.left = curr;
